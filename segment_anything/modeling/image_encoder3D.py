...
class ImageEncoderViT3D(nn.Module):
    def __init__(
        ...
        for i in range(depth):
            block = Blocpatch_embed = PatchEmbed3D(
            kernel_size=patch_size,
            stride=patch_size,
            in_chans=in_chans,
            embed_dim=embed_dim,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 通过每个块，并将每次块的输出保存到outputs列表中
        for blk in self.blocks:
            x = blk(x)
            z = x.permute(0, 4, 1, 2, 3)
            outputs.append(z)  # 保存每个块的输出
...
#
class DB(nn.Module):
    # X: input feature [B, C, H, W, D]
    # lambda: coefficient λ in Eqn (5)
    def forward(self, X, lambda_):
        # spatial size
        lambda_ = lambda_.unsqueeze(2).unsqueeze(3).unsqueeze(4)
        ...
        E_inv = d / (4 * (v.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) + lambda_)) + 0.5
...
class DepthwiseSeparableConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False):
        ...
        self.depthwise = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=in_channels, bias=bias)
...
class Block3D(nn.Module):
    ...
    def __init__(
        ...
        # self.tconv1 = DeformBasicBlock(dim, dim * 2)
        self.tconv1=DepthwiseSeparableConv3d(dim,dim//4,kernel_size=3,padding=1,bias=False)
        self.tconv2 =DepthwiseSeparableConv3d(dim//4, dim, kernel_size=3, padding=1,bias=False)
        self.re=nn.GELU()
        self.drops=nn.Dropout(0.1)
        self.db=DB()
        self.fc11=nn.Linear(dim,dim//8)
        self.fc22=nn.Linear(dim//8,dim//4)
        self.pool = nn.AdaptiveAvgPool3d(dim//4)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # self.r = dim//3  # 低秩维度
        self.r = 192
        self.factor1 = nn.Linear(dim, self.r, bias=False)
        # self.ln=nn.LayerNorm(self.r)
        self.factor2 = nn.Linear(self.r, dim * 3, bias=False)
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.proj = nn.Linear(dim, dim)
        self.re=nn.LeakyReLU()
        self.drop=nn.Dropout(0.1)
        # self.ln2=nn.LayerNorm(dim*3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_factor = self.factor1(x)
        x_factor=self.re(x_factor)
        x_factor = self.factor2(x_factor)
        qkv_original = self.qkv(x)
        # 结合原始qkv操作和低秩操作的结果     # qkv = qkv_original
        # qkv = qkv_original
        qkv = qkv_original + x_factor
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        # self.proj=DeformConv3d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding)
        self.proj = nn.Conv3d(
