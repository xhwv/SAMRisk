...
def get_next_click3D_torch_ritm(prev_seg, gt_semantic_seg):
    ...
    to_point_masks = torch.zeros_like(fn_masks)

    for i in range(gt_semantic_seg.shape[0]):
        fn_mask_single = F.pad(fn_masks[i], (1, 1, 1, 1, 1, 1), 'constant', value=0).to(torch.uint8)
        fp_mask_single = F.pad(fp_masks[i], (1, 1, 1, 1, 1, 1), 'constant', value=0).to(torch.uint8)
        ...
        to_point_masks[i] = to_point_mask
